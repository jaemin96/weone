...
describe("Button", () => {
    ...
    test("button correct classes based on props", () => {
        const {container} = render(<Button primary size={"large"}>Click</Button>);
    ...
    test("handle onClick event", async () => {
        ...
        render(<Button size={"large"} onClick={() => handleClick()}>Click</Button>);
        const buttonElement = screen.getByText(/Click/i);
    ...
    test("snapshot", () => {
        const { asFragment } = render(<Button primary size={"small"}>Click</Button>);
